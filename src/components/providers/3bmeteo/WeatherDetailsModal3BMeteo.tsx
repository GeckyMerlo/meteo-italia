...
interface HourlyWeather3BMeteo {
  time: string;
  condition: string;
  temperature: string;
...
interface WeatherDetailsModal3BMeteoProps {
  isOpen: boolean;
  onClose: () => void;
  provider: string;
  city: string;
...
const WeatherDetailsModal3BMeteo: React.FC<WeatherDetailsModal3BMeteoProps> = ({
  isOpen,
  onClose,
  provider,
  city,
  ...
  const [hourlyData, setHourlyData] = useState<HourlyWeather3BMeteo[]>([]);
  ...
  const loadHourlyData = async () => {
    ...
    try {
      // Converti il giorno selezionato nel formato corretto per 3B Meteo API
      let dayParam: string;
      ...
      const apiEndpoint = `/api/3bMeteo/orari?city=${encodeURIComponent(city)}&day=${dayParam}`;
      ...
      if (response.ok) {
        if (Array.isArray(data) && data.length > 0) {
          ...
          // Caso di array vuoto - non dovrebbe succedere con il nuovo fallback
          setHourlyData([]);
          setError('Nessun dato orario disponibile per questo giorno su 3B Meteo.');
        }
      } else {
        setHourlyData([]);
        setError('Errore nel caricamento dei dati orari da 3B Meteo.');
      }
    } catch (err) {
      console.error('[3B Meteo Modal] Errore nel caricamento dati orari:', err);
      setHourlyData([]);
  ...
  // Weather icon mapping function specific to 3B Meteo
  const getWeatherIcon = (condition: string) => {
    ...
    if (cond.includes('pioggia') || cond.includes('piovoso') || cond.includes('rain') || cond.includes('rovesci')) {
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-6 relative">
          ...
          <div className="flex items-center space-x-3 mb-2">
            <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center p-1">
              <img 
                src="/images/3BMeteoLogo.png" 
                alt="3B Meteo"
                className="w-full h-full object-contain"
              />
            </div>
            <div>
              <h2 className="text-2xl font-bold">3B Meteo</h2>
              <p className="text-blue-100">Previsioni orarie dettagliate</p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div className="flex items-center space-x-2 text-sm text-blue-700 dark:text-blue-300">
                  ...
                  <span>
                    Dati forniti da 3B Meteo - Aggiornato in tempo reale
                  </span>
                </div>
              </div>
            </div>
...
export default WeatherDetailsModal3BMeteo;

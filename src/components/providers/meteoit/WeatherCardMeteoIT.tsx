...
// Sostituisco gli import dei componenti UI con elementi HTML base
...
const WeatherCardMeteoIT: React.FC<WeatherCardMeteoITProps> = ({ weatherData, data, loading }) => {
  ...
  if (loading) {
    return (
      <div className="w-full max-w-md mx-auto animate-pulse bg-white dark:bg-gray-800 rounded-xl shadow-lg">
        <div className="p-6">
  ...
  if (actualData.days && actualData.days.length > 0) {
    return (
      <>
        <div className="w-full max-w-md mx-auto bg-gradient-to-br from-purple-500 to-purple-600 text-white shadow-xl rounded-xl">
              ...
              <div className="grid grid-cols-3 gap-2">
                {actualData.days.slice(1, 4).map((day, index) => (
                  <button
                    ...
                    className="flex flex-col items-center p-2 text-white hover:bg-purple-400/20 h-auto rounded-md transition-colors"
                  >
                    <div className="text-xs opacity-80">
                      {getDayName(index + 1)}
                    </div>

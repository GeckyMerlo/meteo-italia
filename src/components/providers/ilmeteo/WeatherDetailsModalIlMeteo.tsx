...
interface HourlyWeatherIlMeteo {
  time: string;
  condition: string;
  temperature: string;
...
interface WeatherDetailsModalIlMeteoProps {
  isOpen: boolean;
  onClose: () => void;
  provider: string;
  city: string;
...
const WeatherDetailsModalIlMeteo: React.FC<WeatherDetailsModalIlMeteoProps> = ({
  isOpen,
  onClose,
  provider,
  city,
  ...
  const [hourlyData, setHourlyData] = useState<HourlyWeatherIlMeteo[]>([]);
  ...
  const loadHourlyData = async () => {
    ...
    try {
      // Converti il giorno selezionato nel formato corretto per ilMeteo API
      let dayParam: string;
      ...
      const apiEndpoint = `/api/ilMeteo/orari?city=${encodeURIComponent(city)}&day=${dayParam}`;
      
      const response = await fetch(apiEndpoint);
      const data = await response.json();
      
      if (response.ok) {
        // Gestisci diversi formati di risposta dalle API ilMeteo
        let hourlyEntries: HourlyWeatherIlMeteo[] = [];
        ...
        if (hourlyEntries.length > 0) {
          ...
          setHourlyData([]);
          setError('Nessun dato orario disponibile per questo giorno su ilMeteo.');
        }
      } else {
        setHourlyData([]);
        setError('Errore nel caricamento dei dati orari da ilMeteo.');
      }
    } catch (err) {
      console.error('[ilMeteo Modal] Errore nel caricamento dati orari:', err);
      setHourlyData([]);
  ...
  // Weather icon mapping function specific to ilMeteo
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-6 relative">
          ...
          <div className="flex items-center space-x-3 mb-2">
            <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center p-1">
              <img 
                src="/images/ilMeteoLogo.png" 
                alt="ilMeteo"
                className="w-full h-full object-contain"
              />
            </div>
            <div>
              <h2 className="text-2xl font-bold">ilMeteo</h2>
              <p className="text-green-100">Previsioni orarie dettagliate</p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="mt-8 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div className="flex items-center space-x-2 text-sm text-green-700 dark:text-green-300">
                  ...
                  <span>
                    Dati forniti da ilMeteo - Aggiornato in tempo reale
                  </span>
                </div>
              </div>
            </div>
...
export default WeatherDetailsModalIlMeteo;
